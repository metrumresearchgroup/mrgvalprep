...
withr::with_options(list(rbabylon.model_directory = NULL), {
  ...
  test_that("modify_model_field() works correctly [TST-FOO-058]", {
    on.exit({ cleanup() })
  ...
  test_that("modify_model_field() de-duplication works [TST-FOO-059]", {
    on.exit({ cleanup() })
  ...
  test_that("add_tags() and replace_tags() work correctly [TST-FOO-060]", {
    on.exit({ cleanup() })
  ...
  test_that("add_decisions() and replace_decisions() work correctly [TST-FOO-061]", {
    on.exit({ cleanup() })
  ...
  test_that("replace_description() works correctly [TST-FOO-062]", {
    on.exit({ cleanup() })
  ...
  test_that("add_bbi_args() and replace_bbi_args() work correctly [TST-FOO-063]", {
    on.exit({ cleanup() })
  ...
  test_that("add_tags etc. can be chained [TST-FOO-064]", {
    on.exit({ cleanup() })
  ...
  test_that("add_based_on() and replace_based_on() work correctly [TST-FOO-065]", {
    on.exit({ cleanup() })
  ...
  test_that("safe_based_on works on happy path [TST-FOO-066]", {
    on.exit({ cleanup() })
    # test on 1.yaml
  ...
  test_that("safe_based_on works on nested dirs [TST-FOO-068]", {
    on.exit({ cleanup() })
  ...
  test_that("safe_based_on works with absolute path [TST-FOO-069]" , {
    on.exit({ cleanup() })
  ...
  test_that("safe_based_on fails when yaml can't be found [TST-FOO-070]", {
  ...
  test_that("reconcile_yaml() pulls in new tags [TST-FOO-071]", {
    on.exit({ cleanup() })
  ...
  test_that("check_yaml_in_sync() passes when nothing has changed [TST-FOO-072]", {
    new_mod <- read_model(YAML_TEST_FILE)
    expect_invisible(check_yaml_in_sync(new_mod))
  })

  test_that("check_yaml_in_sync() fails when YAML has changed and passes after reconciled [TST-FOO-073]", {
    on.exit({ cleanup() })
  ...
  test_that("add_tags fails if it wasn't re-assigned previously (testing check_yaml_in_sync) [TST-FOO-074]", {
    on.exit({ cleanup() })
  ...
  test_that("submit_model() fails YAML out of sync (testing check_yaml_in_sync) [TST-FOO-075]", {
    on.exit({ cleanup() })
  ...
  test_that("model_summary() fails YAML out of sync (testing check_yaml_in_sync) [TST-FOO-076]", {
    on.exit({ cleanup() })
  ...
  test_that("copy_model_from() fails YAML out of sync (testing check_yaml_in_sync) [TST-FOO-077]", {
    on.exit({ cleanup() })
