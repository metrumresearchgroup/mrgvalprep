...
withr::with_options(list(rbabylon.model_directory = NULL), {
  ...
  test_that("run_log matches reference [MRGVAL-TEST-0100]", {
  ...
  test_that("run_log() works with both yaml and yml [MRGVAL-TEST-0101]", {
  ...
  test_that("run_log() works correctly with nested dirs [MRGVAL-TEST-0102]", {
    log_df <- run_log(MODEL_DIR)
    expect_equal(nrow(log_df), 4)
    expect_equal(ncol(log_df), 8)
  ...
  test_that("config_log() works correctly with nested dirs [MRGVAL-TEST-0103]", {
  ...
  test_that("add_config() works correctly with nested dirs [MRGVAL-TEST-0104]", {
  ...
  test_that("run_log fails after messing with YAML [MRGVAL-TEST-0105]", {
