...
test_that("create_model_object() correctly assigns class [TST-FOO-001]", {
  .mod <- list()
  .mod[[WORKING_DIR]] <- "naw"
...
test_that("create_model_object() fails with non-valid model type [TST-FOO-002]", {
  .mod <- list()
  .mod[[WORKING_DIR]] <- "naw"
...
test_that("create_model_object() fails with non-valid model file extension [TST-FOO-003]", {
  .mod <- list()
  .mod[[WORKING_DIR]] <- "naw"
  .mod[[YAML_YAML_NAME]] <- "naw"
  .mod[[YAML_YAML_MD5]] <- "naw"
...
test_that("create_model_object() errors if keys are missing [TST-FOO-004]", {
  .mod <- list()
  .mod[[WORKING_DIR]] <- "naw"
  .mod[[YAML_MOD_TYPE]] <- "naw"
  #.mod[[YAML_DESCRIPTION]] <- "naw"
...
test_that("create_summary_object() correctly assigns class [TST-FOO-005]", {
  .sum <- list()
...
test_that("create_summary_object() errors if keys are missing [TST-FOO-006]", {
  .sum <- list()
  #.sum[[SUMMARY_DETAILS]] <- "naw"
  .sum$naw <- "naw"
...
test_that("create_process_object() correctly assigns class [TST-FOO-007]", {
  .proc <- list()
...
test_that("create_process_object() errors if keys are missing [TST-FOO-008]", {
  .proc <- list()
  .proc[[PROC_PROCESS]] <- "naw"
  .proc[[PROC_STDOUT]] <- "naw"
  .proc[[PROC_BBI]] <- "naw"
...
test_that("create_run_log_object() correctly assigns class [TST-FOO-009]", {
...
test_that("create_run_log_object() errors if keys are missing [TST-FOO-010]", {
  .log_df <- tibble::tibble(
    !!ABS_MOD_PATH      := c("naw", "dawg"),
    !!YAML_YAML_MD5     := c("naw", "dawg"),
    !!YAML_MOD_TYPE     := c("naw", "dawg"),
...
test_that("create_run_log_object() errors if ABS_MOD_PATH is not character [TST-FOO-011]", {
  .log_df <- tibble::tibble(
    !!ABS_MOD_PATH      := c(1, 2),
    !!YAML_YAML_MD5     := c("naw", "dawg"),
    !!YAML_MOD_TYPE     := c("naw", "dawg"),
...
test_that("create_run_log_object() errors if ABS_MOD_PATH is not unique [TST-FOO-012]", {
  .log_df <- tibble::tibble(
...
test_that("create_run_log_object() errors if ABS_MOD_PATH has missing values [TST-FOO-013]", {
  .log_df <- tibble::tibble(
