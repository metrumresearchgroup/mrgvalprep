...
withr::with_options(list(rbabylon.model_directory = NULL), {
  ...
  test_that("get_path_from_object.default() builds the right path [mrgval-TEST-0033]", {
    expect_identical(get_path_from_object(MOD1 , YAML_MOD_PATH), normalizePath(CTL_TEST_FILE))
  })

  test_that("get_path_from_object.character() builds the right path [mrgval-TEST-0034]", {
  ...
  test_that("get_path_from_object.character() fails with vector [mrgval-TEST-0035]", {
    expect_error(get_path_from_object(c("naw", "dawg") , YAML_MOD_PATH), regexp = "only scaler values are permitted")
  })

  test_that("get_path_from_object.character() .check_exists works [mrgval-TEST-0036]", {
  ...
  test_that("get_path_from_object.bbi_run_log_df() builds the right paths [mrgval-TEST-0037]", {
  ...
  test_that("get_path_from_object() errors on missing keys [mrgval-TEST-0038]", {
  ...
  test_that("get_path_from_object() errors on vector field [mrgval-TEST-0039]", {
    .test_list <- list(naw = c(1,2))
    .test_list[[WORKING_DIR]] <- "/fake/path"
    expect_error(get_path_from_object(.test_list, "naw"), regexp = "Expected a scaler value")
  })

  test_that("get_path_from_object() errors on fake path [mrgval-TEST-0040]", {
    .test_list <- list(naw = 1)
    .test_list[[WORKING_DIR]] <- "/fake/path"
  ...
  test_that("get_model_path() builds the right path [mrgval-TEST-0041]", {
    expect_identical(get_model_path(MOD1), normalizePath(CTL_TEST_FILE))
  })

  test_that("get_output_dir() builds the right path [mrgval-TEST-0042]", {
    expect_identical(get_output_dir(MOD1), normalizePath(OUTPUT_DIR))
  })

  test_that("get_yaml_path() builds the right path [mrgval-TEST-0043]", {
  ...
  test_that("is_valid_nonmem_extension() works [mrgval-TEST-0044]", {
  ...
  test_that("is_valid_yaml_extension() works [mrgval-TEST-0045]", {
  ...
  test_that("find_model_file_path returns correct ctl path [mrgval-TEST-0046]", {
    expect_identical(find_model_file_path(CTL_TEST_FILE), CTL_TEST_FILE)
  })

  test_that("find_model_file_path prefers ctl path [mrgval-TEST-0047]", {
    expect_identical(find_model_file_path(MOD_TEST_FILE), CTL_TEST_FILE)
  })

  test_that("find_model_file_path returns ctl path when no path found [mrgval-TEST-0048]", {
    mod_file <- "data/1.mod"
    expect_identical(
  ...
  test_that("find_model_file_path returns mod path when only path found [mrgval-TEST-0049]", {
    mod_file <- "data/1.mod"
  ...
  test_that("find_yaml_file_path returns correct yaml path [mrgval-TEST-0050]", {
    expect_identical(find_yaml_file_path(YAML_TEST_FILE), YAML_TEST_FILE)
  })

  test_that("find_yaml_file_path returns correct yml path [mrgval-TEST-0051]", {
  ...
  test_that("find_yaml_file_path errors when no file found [mrgval-TEST-0052]", {
    expect_error(find_yaml_file_path(NEW_MOD2), regexp = FIND_YAML_ERR_MSG)
  })

  test_that("find_yaml_file_path errors when two files found [mrgval-TEST-0053]", {
  ...
  test_that("combine_directory_path() builds the expected path .directory [mrgval-TEST-0054]", {
    res_path <- combine_directory_path(MODEL_DIR, ctl_ext(MOD_ID))
    expect_identical(res_path, ABS_CTL_PATH)
  })

  test_that("combine_directory_path() builds the expected path with NULL .directory [mrgval-TEST-0055]", {
    res_path <- combine_directory_path(.directory = NULL, CTL_TEST_FILE)
    expect_identical(res_path, ABS_CTL_PATH)
  })

  test_that("combine_directory_path() builds fake .path in real .directory [mrgval-TEST-0056]", {
    res_path <- combine_directory_path(MODEL_DIR, CTL_TEST_FILE)
    expect_identical(res_path, FAKE_CTL_PATH)
  })

  test_that("combine_directory_path() errors with fake .directory [mrgval-TEST-0057]", {
