...
withr::with_options(list(rbabylon.model_directory = NULL), {
  ...
  test_that("get_based_on works happy path model object [TST-FOO-020]" , {
    on.exit({ cleanup() })
  ...
  test_that("get_based_on works happy path character [TST-FOO-021]" , {
    on.exit({ cleanup() })
  ...
  test_that("get_based_on.character() fails with vector [TST-FOO-022]", {
    expect_error(get_based_on(c("naw", "dawg")), regexp = "only scaler values are permitted")
  })

  test_that("get_based_on works happy path run_log tibble [TST-FOO-023]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_based_on constructs ancestry manually [TST-FOO-024]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_based_on .check_exists=TRUE errors if model is gone [TST-FOO-025]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_based_on() behaves correctly on missing keys [TST-FOO-026]", {
  ...
  test_that("get_model_ancestry works happy path model object [TST-FOO-027]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_model_ancestry works happy path character [TST-FOO-028]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_model_ancestry.character() fails with vector [TST-FOO-029]", {
    expect_error(get_model_ancestry(c("naw", "dawg")), regexp = "only scaler values are permitted")
  })

  test_that("get_model_ancestry works happy path run_log tibble [TST-FOO-030]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_model_ancestry errors if model is gone [TST-FOO-031]" , {
    on.exit({ cleanup() })
    create_all_models()
  ...
  test_that("get_model_ancestry works on run_log tibble with more complicated ancestry [TST-FOO-032]" , {
    on.exit({ cleanup() })
    create_all_models()
