...
extract_r_package_info <- function(path) {
  ...
  release_file <- parse_latest_release(
    ...
    .n,
    tempfile(pattern = glue("{.n}-{.v}-release"), fileext = ".md")
  )
...
parse_latest_release <- function(news_path, package_name, out_path) {
  assert_string(news_path)
  assert_string(out_path)

  news_lines <- read_lines(news_path)
  releases <- grep(glue("^# {package_name}"), news_lines)
  if (length(releases) == 0) {
    stop(glue("No top-level headings matching '# <package name>' found in {news_path}"))
  }
  ...
  write_lines(release_lines, out_path)
  return(out_path)
}
...
find_spec_files <- function(val_dir) {
  ...
  if (length(st_files) == 0) {
    stop(glue("can't find any valid stories YAML files in {val_dir}"))
  }

  req_files <- fs::dir_ls(val_dir, regexp = "requirements\\.ya?ml")

  if (length(req_files) == 0) {
    stop(glue("can't find any valid requirements YAML files in {val_dir}"))
  }
