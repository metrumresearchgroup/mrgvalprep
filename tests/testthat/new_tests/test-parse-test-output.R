...
test_that("parse_testthat_list_reporter() returns expected tibble [TST-FOO-015]", {
...
test_that("parse_testthat_list_reporter() optionally rolls up ID's [TST-FOO-016]", {
  test_res <- readRDS(system.file("test-refs", "test-parse-test-output-ListReporter.RDS", package = "mrgvalprep"))
...
test_that("trailing colon is stripped when rolling up IDs [TST-FOO-017]", {
  df <- tibble::tribble(
...
test_that("parse_golang_test_json() happy path [TST-FOO-018]", {
...
test_that("parse_golang_test_json() missing test id [TST-FOO-019]", {
...
test_that("parse_golang_test_json() two test ids for single test [TST-FOO-020]", {
...
test_that("parse_test_id() returns NA if there is no ID [TST-FOO-021]", {
  cases <- list(
    # input expected
    c("", NA),
    c("foo", NA),
...
test_that("parse_test_id() can parse IDs [TST-FOO-022]", {
  cases <- list(
    # input expected
...
test_that("parse_test_id() extracts only the first match [TST-FOO-023]", {
  expect_equal(parse_test_id("[FOO-BAR-001] [FOO-BAR-002] "),
               "FOO-BAR-001")
})

test_that("strip_test_id() returns string without a bracketed test ID as is [TST-FOO-024]", {
  cases <- list(
    # input expected
    c("", ""),
    c("foo", "foo"),
...
test_that("strip_test_id() strips only specified test ID [TST-FOO-025]", {
...
test_that("strip_test_id() strips only the first match [TST-FOO-026]", {
  expect_equal(strip_test_id("a[FOO-BAR-001]b[FOO-BAR-001]",
                             "FOO-BAR-001"),
               "ab[FOO-BAR-001]")
})

test_that("strip_test_id() collapses spaces [TST-FOO-027]", {
  cases <- list(
    # input expected
    c("[FOO-001]", "[FOO-001]"),
