test_that("get_sys_info() returns date and default values [TST-FOO-006]", {
  res <- get_sys_info()
...
test_that("get_sys_info() returns system info [TST-FOO-007]", {
  res <- get_sys_info(sys_info = TRUE)
  expect_equal(names(res$info$sys), c("sysname", "version", "release", "machine"))
})

test_that("get_sys_info() writes to json [TST-FOO-008]", {
  tmp_file <- tempfile()
...
test_that("get_sys_info() captures environment variables [TST-FOO-009]", {
...
test_that("get_sys_info() captures sessionInfo [TST-FOO-010]", {
