...
This example shows how to validate software that contains both automated _and_ manual tests, as well as how to pull Stories and Requirements from Googlesheets.
...
devtools::test(reporter = testthat::ListReporter) %>% # run testthat tests suite
  ...
  readr::write_csv("auto_tests/test_res.csv")         # write tibble for .csv file

# capture system information for test suite run
get_sys_info(
  out_path = "auto_tests/test_res.json",
...
# pull stories and requirements from googlesheets
...
mrgvalidate::create_validation_docs(
  ...
  man_test_dir = "man_tests",       # dir containing manual test outputs
...
This section shows how to include the results of both automated and manual tests in your validation documents. It is never required (by the package) to use _both_ of these. You can use either one or both as is appropriate for the software you're validating.
...
Currently, `mrgvalprep` only has a formatter for R's `testthat` package ([`parse_testthat_list_reporter()`](https://metrumresearchgroup.github.io/mrgvalprep/reference/parse_testthat_list_reporter.html)), though we are planning to add more functions to take the output from other test frameworks (for example `go test --json` output) and return a tibble that conforms to [?mrgvalidate::input_formats](https://metrumresearchgroup.github.io/mrgvalidate/reference/input_formats.html).
...
This will run your full test suite, format the output appropriately, and then write the formatted output to a `.csv` for [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html) to consume. 
...
_Note:_ [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html) will expect to find a `.csv` and `.json` with the same basename for each test suite, so be sure that what you pass to `create_sys_info(out_path)` matches the `.csv` of test ouputs that you wrote in the previous section.

### Manual test results

Sometimes manual tests are required to fully validate your software. [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html) allows you to pass a directory containing `.md` files which contain the "results" of these manual tests (typically things like screenshots and accompanying narratives). The required format for this directory, as well as the `.md` and image files in it, is described in [`?mrgvalidate::input_formats`](https://metrumresearchgroup.github.io/mrgvalidate/reference/input_formats.html#arguments).
...
The format of these Googlesheets is described in [the `read_spec_gsheets()` docs](https://metrumresearchgroup.github.io/mrgvalprep/reference/read_spec_gsheets.html#details). At a high-level, the Stories sheet must have the following columns:
...
Calling [`read_spec_gsheets()`](https://metrumresearchgroup.github.io/mrgvalprep/reference/read_spec_gsheets.html) will pull the data in your sheet(s) into a tibble that can be passed directly to [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html).
...
You will have `.csv` and `.json` files of automated test outputs, from your [`validate_tests()`](https://metrumresearchgroup.github.io/mrgvalprep/reference/validate_tests.html) call above, in the `auto_tests` directory, as well as `.md` capturing your manual tests in the `man_tests` directory. Simply pass these paths, along with your tibble of Stories, into [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html).
...
_Formatting note:_ You may want to manually adjust the column widths of the tables in these documents for better readability, especially so that each "Test Id" column value is on its own line. Unfortunately, it is not possible to programmatically specify these column widths. However, it is possible to pass `.docx` "style reference templates" via the [`style_dir` argument](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html#arguments) to [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html).
...
You will then put the resulting `.csv` and `.json` files into a directory that will be passed to the `auto_test_dir` argument of [`mrgvalidate::create_validation_docs()`](https://metrumresearchgroup.github.io/mrgvalidate/reference/create_validation_docs.html).
